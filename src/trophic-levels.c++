 ...
 *  defining an inter-level connection as one exceeding a given threshold,
 *  expressed as a multiple of the SD of the community matrix. Numbers of levels
 *  depend on this threshold, but for reasonably large values (> 2 * SD),
 *  numbers of levels range between 2 and 6. Proportions of actual networks
 *  generated for various numbers of trophic levels are given below for
 *  thresholds defined by several multiples of SD.
 ***************************************************************************/
        ...
        base_generator_type & generator)
{
    ...
    for (int i=0; i<nSpecies; i++) 
    {
        if (troph_levels (i) == INT_MIN) 
            std::cout << "*";
        else 
            std::cout << troph_levels (i);
        std::cout.flush();
    }
    std::cout << std::endl;
    */
